...
{
  ?>
    <div class="description_single_prod">
      <?php the_content(); ?>
    </div>
  </div>
  <?php
  ...
  ?>
    <div class="product_left">
      ...
      <div class="product_bottom">
        ...
        <div class="product_bottom-button_box">
          <a class="like add-to-wishlist-button" data-product-id="<?php echo get_the_ID(); ?>">
            <!-- <img src="http://smaug2cq.beget.tech/wp-content/themes/ohpirogi/assets/images/m-like.png" alt=""> -->
          </a>
...
function true_add_custom_billing_field($fields)
{
  ...
  for ($i = 0; $i < 14; $i++) {
    $item_date = date('d', strtotime('+' . $i . ' day')) . ' ' . $manth_names[date('n')];
    $arr_date[$item_date] = $item_date;
  }

  $h_time_now = date('G', strtotime('+' . 9 . ' hours'));

  if (($h_time_now < 22 && $h_time_now > 4) && date('d') == (current_datetime()->format('d'))) {
    for ($i = 0; $i < 24; $i++) {
      if (($h_time_now + $i) > 9 && ($h_time_now + $i) < 22) {
        $h_time = date('H', strtotime('+' . 9 + $i . ' hours')) . ":00 - " . date('H', strtotime('+' . 10 + $i . ' hours')) . ":00";
  ...
  // $fields = array_slice($fields, 0, 3, true) + array_slice($fields, 2, NULL, true);
  ...
  if (get_user_meta(get_current_user_id(), 'date_of_birth', true) != '') {
    ...
    for ($i = -3; $i <= 3; $i++) {
      ...
      if ($for_date == ($date_clients[0] . '.' . $date_clients[1])) {
        echo '<p><strong>У клиента день рождения !!!</strong></p>';
      }
    }
  }
}
...
{
  wp_enqueue_script('custom-login-script', get_template_directory_uri() . '/js/custom-login.js', array('jquery'), null, true);
...
add_action('woocommerce_single_product_summary', function () {
  // echo 'text';
  $tth_fields = (carbon_get_post_meta($GLOBALS['id_pro_carrent'], 'crb_tth')) ? carbon_get_post_meta($GLOBALS['id_pro_carrent'], 'crb_tth') : '';
  if ($tth_fields) {
    ?>
      <table class="tth_table">
        <caption class="tth_table_title">Технические характеристики</caption>
        <?php foreach ($tth_fields as $item) { ?>
          <tr>
            <th>
              <?php echo $item['title']; ?>
            </th>
            <td>
              <?php echo $item['crb_tth_vel']; ?>
            </td>
          </tr>
        <?php } ?>
      </table>
  ...
  $options_fields = (carbon_get_post_meta($GLOBALS['id_pro_carrent'], 'crb_options')) ? carbon_get_post_meta($GLOBALS['id_pro_carrent'], 'crb_options') : '';
  if ($options_fields) {
    ?>
      <table class="options_table">
        <caption class="options_table_title">Комплектация</caption>
        <?php foreach ($options_fields as $item) { ?>
          <tr>
            <td>
              <?php
              if ($item['crb_options_link'] != '') {
                echo "<a href='{$item['crb_options_link']}'>{$item['crb_options_name']}</a>";
              } else {
                echo $item['crb_options_name'];
              }
              ?>
              <?php // echo $item['crb_options_name']; ?>
            </td>
            <td>
              <?php echo $item['crb_options_size']; ?>
            </td>
            <td>
              <?php echo 'от: ' . $item['crb_options_price'] . ' &#x20bd;'; ?>
            </td>
          </tr>
        <?php } ?>
      </table>
  ...
  $crb_scheme_and_size_image = (carbon_get_post_meta($GLOBALS['id_pro_carrent'], 'crb_scheme_and_size_image')) ? carbon_get_post_meta($GLOBALS['id_pro_carrent'], 'crb_scheme_and_size_image') : '';
  if ($crb_scheme_and_size_image) {

    ?>
      <h4>Схема и размеры</h4>
      <img src="<?= $crb_scheme_and_size_image; ?>" alt="">
      <?php
  }
}, 50);
...
add_action('woocommerce_after_single_product_summary', function () {
  // echo 'text';
  // echo carbon_get_post_meta($GLOBALS['id_pro_carrent'], 'crb_photos_of_interiors');

  $crb_photos_of_interiors_fields = (carbon_get_post_meta($GLOBALS['id_pro_carrent'], 'crb_photos_of_interiors')) ? carbon_get_post_meta($GLOBALS['id_pro_carrent'], 'crb_photos_of_interiors') : '';
  if ($crb_photos_of_interiors_fields) {
    ?>
      <h4>Интерьерные фотографии</h4>
      <div class="interiors_img_box">
        <?php foreach ($crb_photos_of_interiors_fields as $item) { ?>

          <img src="<?= $item['crb_photo']; ?>" alt="">
  ...
  $crb_pohozhee_fields = (carbon_get_post_meta($GLOBALS['id_pro_carrent'], 'crb_pohozhee_in_admin')) ? carbon_get_post_meta($GLOBALS['id_pro_carrent'], 'crb_pohozhee_in_admin') : '';
  if ($crb_pohozhee_fields) {
    ?>
      <div class="pohozhee_box">
        <?php foreach ($crb_pohozhee_fields as $item) { ?>

          <?php echo do_shortcode('[my_product_top_ten product_cat="' . $item['crb_set_discount_to_term'] . '" title="' . $item['crb_options_name'] . '"]'); ?>
